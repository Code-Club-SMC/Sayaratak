...
import { BASE_URL, ensureAbsoluteUrl, buildEntityUrl } from "../lib/url";
import {
	SLUG_KEYED_ENTITY_TYPES,
	ID_KEYED_ENTITY_TYPES,
	type ResolvableEntityType,
} from "../lib/entity-resolver";
import { NotFoundError, GoneError, BadRequestError } from "../lib/errors";
import type { SupportedLocale } from "../lib/i18n";

export const DEFAULT_OG_IMAGE = `${BASE_URL}/sayaratak-og-default.jpg`;
export const ALL_VALID_TYPES = [...SLUG_KEYED_ENTITY_TYPES, ...ID_KEYED_ENTITY_TYPES] as const;
export type ValidSeoType = ResolvableEntityType;
...
export const seoService = {
	async getMetadata(rawType: string, identifier: string, locale: SupportedLocale) {
		const type = rawType.toLowerCase() as ValidSeoType;

		if (!ALL_VALID_TYPES.includes(type)) {
			throw new BadRequestError(`Invalid SEO entity type: ${type}`, "INVALID_ENTITY_TYPE");
		}
		...
		if (type === "listing" || type === "vehicle") {
			...
			if (Array.isArray(listing.media)) {
				for (const item of listing.media) {
					if (typeof item === "string") rawPhotos.push(ensureAbsoluteUrl(item));
						...
						rawPhotos.push(ensureAbsoluteUrl(item.url));
				}
			}
			...
			const cityName =
				locale === "ar"
					? (listing.cityNameAr ?? listing.cityNameEn)
					: (listing.cityNameEn ?? "Sudan");
			const makeName =
				locale === "ar"
					? (listing.makeNameAr ?? listing.makeNameEn)
					: listing.makeNameEn;
			const yearPrefix = listing.year
				? locale === "ar"
					? `موديل ${listing.year} `
					: `${listing.year} `
				: "";
		...
		// ----------------- B. DEALERSHIP -----------------
			...
			const cityName =
				locale === "ar"
					? (dealer.cityNameAr ?? dealer.cityNameEn)
					: (dealer.cityNameEn ?? "Sudan");
			const dealerName =
				dealer.name || (locale === "ar" ? "معرض سيارات" : "Car Dealership");
		...
		// ----------------- C. WORKSHOP -----------------
			...
			const cityName =
				locale === "ar"
					? (ws.cityNameAr ?? ws.cityNameEn)
					: (ws.cityNameEn ?? "Sudan");
			const wsName =
				ws.name || (locale === "ar" ? "ورشة صيانة" : "Auto Workshop");
		...
		// ----------------- D. MECHANIC -----------------
			...
			const cityName =
				locale === "ar"
					? (mech.cityNameAr ?? mech.cityNameEn)
					: (mech.cityNameEn ?? "Sudan");
			const mechName =
				mech.name || (locale === "ar" ? "فني سيارات" : "Mechanic");
			const spec =
				mech.specialization || (locale === "ar" ? "صيانة سيارات" : "Automotive Specialist");
			...
			jsonLd = {
				...
				...(mech.yearsExperience
					? { knowsAbout: `${mech.yearsExperience} years experience in automotive repair` }
					: {}),
			};
		}
		...
		// ----------------- E. CATEGORY (SLUG-KEYED) -----------------
			...
			jsonLd = {
				...
				name: resolvedTitle,
				description: resolvedDescription,
				url: buildEntityUrl(type, identifier, locale),
			};
		}
		...
		// ----------------- F. MAKE (SLUG-KEYED) -----------------
			...
			if (!mk) {
				throw new NotFoundError("Make not found", "MAKE_NOT_FOUND");
			}
		...
		const canonical = buildEntityUrl(type, identifier, locale);

		const alternates = [
			{ hreflang: "en" as const, href: buildEntityUrl(type, identifier, "en") },
			{ hreflang: "ar" as const, href: buildEntityUrl(type, identifier, "ar") },
			{ hreflang: "x-default" as const, href: buildEntityUrl(type, identifier, "en") },
		];
	...
	async generateSubSitemapXml(name: string) {
		const rawName = name.replace(/\.xml$/, "");
